...
$row1=$result1->fetch_assoc();
   ...
   $sql2="SELECT  count(id) as id3 FROM books WHERE delete_status='0'";
...
$sql5="SELECT p.name, COUNT(s.product_id) AS totalSold, SUM(s.qty) AS totalQty
                   FROM sales1 s
                   LEFT JOIN books p ON p.id = s.product_id 
...
$sql6="SELECT s.id,s.qty,s.price,s.date,p.name
       FROM sales1 s
    LEFT JOIN books p ON s.product_id = p.id
    where s.delete_status=0
    GROUP BY s.product_id
...
<h6 class="text-white m-b-0">Books Available</h6>
</div>
<div class="col-4 text-right">
...
<h6 class="text-white m-b-0">Book Sold</h6>
 </div>
<div class="col-4 text-right">
